...
# --------------------------------------------------------------------------------------
# In app.py, replace your current agent_heartbeat function with this one:
...
def agent_heartbeat():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    # This query now correctly uses UTC time for the timestamp
    cur.execute("""
...
# --------------------------------------------------------------------------------------
# In app.py, REPLACE your server_dashboard function with this debug version:

# In app.py, REPLACE the entire "SERVER DASHBOARD" section with this:
# --------------------------------------------------------------------------------------
# SERVER DASHBOARD
# --------------------------------------------------------------------------------------
...
@app.route("/server_dashboard/data")
def server_dashboard_data():
    """This function provides background data updates for the smooth refresh."""
    if "user" not in session:
        return jsonify({"error": "Unauthorized"}), 401
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        SELECT a.* FROM agents a INNER JOIN (
    ...
    agents = cur.fetchall()
    conn.close()
    unique_ips_count = len({agent["ip_address"] for agent in agents})
    latest_heartbeat = agents[0]["last_heartbeat"] if agents else None
    now_utc = datetime.now(timezone.utc)
    for agent in agents:
